...
if data.empty:
    ...
    with comparison:
        ...
        data_train = pd.DataFrame(data.Close_[0:int(len(data) * 0.80)])
        data_test = pd.DataFrame(data.Close_[int(len(data) * 0.80):])
    
        # Price comparison with moving averages (MA50, MA100, MA200)
        st.subheader(f'Price of {ticker} vs MA50')
        ma_50_days = data.Close_.rolling(50).mean()
        fig1 = plt.figure(figsize=(8, 6))
        plt.plot(ma_50_days, 'r')
        plt.plot(data.Close_, 'g')
        plt.show()
        st.pyplot(fig1)
    
        st.subheader(f'Price of {ticker} vs MA50 vs MA100')
        ma_100_days = data.Close_.rolling(100).mean()
        ...
        ma_200_days = data.Close_.rolling(200).mean()
